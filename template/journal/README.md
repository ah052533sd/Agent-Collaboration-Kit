...
2. **注入上限**——单文件 6000 字符、最多 3 天；截断和被省略的文件都显式标注，提示 Agent 主动补读，而不是误以为看全了。**超预算按条目整条丢弃，不按字符拦腰截断**，且被丢的条目留下标题 + 产物路径。拦腰截断会把一条判断从句子中间切开，读的人分不清是没写完还是被切了；更严重的是产物路径写在正文里，一截就没了——实测一天 28 条语义条目 30876 字符，三条同主题条目 4295 字符全被压成三行标题，另一方据此**定位到了错的产物且不自知**。注入总量超过约 9000 字符时开头另附一条补读警示：Claude Code 会把超过约 10KB 的注入持久化成文件、上下文只留约 2KB 预览，警示保证「必须 Read 全文」的指令落在预览窗口内。
...
- **空转会话不写信封**——HEAD 未变、工作区相对**会话起点**无变化（起点指纹由 SessionStart 记录；`journal/` 与 `.journal-state/` 的改动不计，前者可能是对方刚写的）、且**本次会话**没写过语义条目时直接跳过。判「变化」不判「脏」：产物长期不 commit 的使用模式下「脏」恒为真，实测连 transcript 都没有的空会话也写了信封。判定精确到会话，**不是精确到当天**：按天判断会在当天写过第一条之后失效，之后每个会话都被判成「干过活」，实测一天堆出 25 条信封对 4 条语义。起点指纹缺失时（漏跑 SessionStart、旧格式状态文件）**不退回看工作区**，只认 HEAD 移动或本会话语义条目——退回看「脏」曾在一分钟内写出 12 条重复信封（2026-07-30 实测）。
