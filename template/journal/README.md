...
- **空转会话不写信封**——HEAD 未变、工作区相对**会话起点**无变化（起点指纹由 SessionStart 记录；`journal/` 与 `.journal-state/` 的改动不计，前者可能是对方刚写的）、且**本次会话**没写过语义条目时直接跳过。判「变化」不判「脏」：产物长期不 commit 的使用模式下「脏」恒为真，实测连 transcript 都没有的空会话也写了信封。判定精确到会话，**不是精确到当天**：按天判断会在当天写过第一条之后失效，之后每个会话都被判成「干过活」，实测一天堆出 25 条信封对 4 条语义。
