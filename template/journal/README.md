...
`--session-id` 请尽量带上：`end.py` 靠它精确判断本次会话是否空转（漏传时退回比对会话起始时间）。字段可重复传入。生成的条目形如：
...
| 义务提醒（每轮） | 同上，`UserPromptSubmit` 分支，约 280 字符 | 同上，`UserPromptSubmit` 分支，约 280 字符 |
...
2. **注入上限**——单文件 6000 字符、最多 3 天；截断和被省略的文件都显式标注，提示 Agent 主动补读，而不是误以为看全了。
3. **每轮提醒极短**——无新条目时约 280 字符，只说「有关键判断就立即追加」。20 轮约 5.6k 字符，比一次性全量注入还少。只有对方在你会话中途写了条目时才变长，且水位前移保证同一条不重复。
